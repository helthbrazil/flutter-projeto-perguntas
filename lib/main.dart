...
import 'package:projeto_perguntas/toast_util.dart';
...
class _PerguntaAppState extends State<PerguntaApp> {
  ...
  final List<Map<String, dynamic>> _perguntas = const [
    ...
    {
      'texto':
          'Que animal da fauna brasileira está retratado na nota de dez reis?',
      'respostas': [
  ...
  void _responder(int index) {
    ...
    if (temPerguntaSelecionada) {
      int notaResposta = _perguntas[aux]['respostas'][index - 1]['nota'];
      somaNotas += notaResposta;

      if (notaResposta == 1) {
        ToastUtil.toast('Resposta correta');
      } else {
        ToastUtil.toast('Resposta errada');
      }
      setState(() {
